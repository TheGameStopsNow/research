...
> [!NOTE]
> **Cross-reference: Paper V.** The Failure Accommodation Waterfall (Paper V, §3.4) maps the complete lifecycle of the FTD obligations that Delta Laundering sanitizes. Each FTD enters a 15-node regulatory cascade from T+3 (5.4× enrichment) through T+40 (40.3× enrichment), with phantom OI as its measurable exhaust. The T+3 scienter evidence (Paper V, §3.5) — preemptive synthetic locate manufacturing *before* regulatory deadlines — is the settlement-layer counterpart to the catalyst-locked activation documented here. Delta Laundering (this section) describes the *mechanism* by which the Shadow Algorithm clears specific FTDs; the Waterfall (Paper V) describes the *regulatory pipeline* through which all FTDs propagate if not cleared.
...
Paper III of this series presents the regulatory and policy framework: specific FINRA CAT queries, proposed exchange rule amendments, and the broader implications of the Long Gamma Default for market structure reform. Paper V extends the FTD analysis (§4.2.6) into a complete settlement lifecycle map — the Failure Accommodation Waterfall — tracing delivery failures through 15 regulatory nodes from T+3 to terminal death at T+45.
...
22. U.S. Securities and Exchange Commission. "Fails-to-Deliver Data." Published semi-monthly. [sec.gov/data-research/sec-markets-data/fails-deliver-data](https://www.sec.gov/data-research/sec-markets-data/fails-deliver-data)
