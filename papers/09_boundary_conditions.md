...
2. **Equity failures Granger-cause Treasury failures.** Formal VAR-based Granger causality tests on 215 weeks of NY Fed primary dealer FTD data demonstrate that GME settlement failures predict Treasury settlement fails at all tested lags (1–6 weeks). The reverse relationship is not statistically significant.
...
4. **Cancelled CUSIPs create permanent zombie obligations.** BBBY FTDs persisted for 824 days after the CUSIP was cancelled, with 31 unique observation values confirming active failure cycling on a security that cannot be settled.
...
The result is unambiguous: **GME Granger-causes Treasury fails unidirectionally**. The relationship is significant at all tested lags (1–6 weeks, $p < 0.03$ at every lag), while the reverse direction is not significant ($p > 0.23$ at all lags). No other equity in the sample shows a significant causal relationship with Treasury fails.
...
The 31 unique FTD values are critical: they prove that the figure reported to the SEC is not a frozen cumulative balance reprinted on each reporting date. The CUSIP is *still actively failing*—settlement obligations are cycling through the Obligation Warehouse with no available settlement mechanism, because the underlying company no longer exists.

This constitutes the most direct evidence in the series for the Obligation Warehouse sealing hypothesis: delivery failures on borrow-constrained securities can, through corporate action, become permanent zombie obligations that sit indefinitely outside the CNS system.
...
![Figure 9: BBBY: 824 days of FTDs on a cancelled CUSIP. Top panel shows the SEC-reported outstanding FTD balance. Bottom panel shows day-to-day changes, with 43% exceeding the 10,000-share block-size threshold.](figures/p9_f09_bbby_zombie.png)
...
2. **Across asset classes** (Section 3): Equity FTDs Granger-cause Treasury settlement fails at a 1-week lag ($F = 19.20$, $p < 0.0001$), establishing a cross-market contagion channel unique to GME—no other equity in a seven-ticker sample produces a significant result.
...
4. **Across time** (Section 5): Corporate actions can *permanently* seal obligations in the Obligation Warehouse, creating zombie FTDs that persist for years after CUSIP cancellation (BBBY: 824 days and counting).
...
| Obligation sealing | Permanent | CUSIP cancellation | BBBY 824-day zombie FTDs |
...
![Figure 16: Settlement failure contagion architecture: four channels radiating outward from GME: lateral to KOSS, vertical to U.S. Treasuries, cross-border to European CSDs, and zombie persistence on BBBY's cancelled CUSIP.](figures/p9_f16_contagion_flow.png)
...
|---|---|---|---|---|---|
| (a) | GME→Treasury Granger is spurious macro noise | 25% | Multi-ticker Granger control (7 equities) | **Rejected**: Only GME significant ($F = 19.20$, $p < 0.0001$). Next highest $F = 2.38$ (not significant). If shared macro noise, all equities would Granger-cause Treasury fails. | $< 5\%$ |
...
| (c) | BBBY zombie FTDs are admin rounding errors | 10% | Block-size delta analysis (31 observations) | **Strongly disfavored**: 0% admin-noise deltas ($< \pm 100$), 43% block-sized ($\geq \pm 10{,}000$). Alternating injection/extraction pattern. | $\sim 3\%$ |
...
**Combined null probability**: Under independence, the probability that *all five* findings are simultaneously spurious is $0.05 \times 0.03 \times 0.03 \times 0.05 \times 0.08 < 0.0003\%$. Even under generous correlation assumptions (shared macro environment), the joint null remains below $0.03\%$.

The Granger causality control test (a) merits particular emphasis. By repeating the identical Granger test on seven equities—GME, AMC, KOSS, XRT, AAPL, MSFT, TSLA—against the same Treasury FTD series, we demonstrate that the causal channel is *specific to GME*. The F-statistic ratio of 8.1:1 (GME vs. next-strongest equity) eliminates the shared-macro-factor confound: if both GME FTDs and Treasury fails were simply responding to the same liquidity conditions, any equity with comparable FTD volatility would produce a similar Granger result. None does.
