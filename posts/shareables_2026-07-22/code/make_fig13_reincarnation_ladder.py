#!/usr/bin/env python3
"""Render the shareables variant of the $50 reincarnation ladder.

Source data: posts/claim_tests/strike50_reincarnation_2026-07-22/data/ (frozen
hunt package, untouched). Only change vs the hunt figure: title uses a colon
instead of an em-dash for public-facing style. Output:
posts/shareables_2026-07-22/figures/fig13_reincarnation_ladder.png
"""
...
OUT = REPO / "posts/shareables_2026-07-22/figures/fig13_reincarnation_ladder.png"
...
def shade_eras(ax, y_frac=0.985):
    for e, (a, b) in ERA_SPANS.items():
        ax.axvspan(pd.Timestamp(a), pd.Timestamp(b), color=ERA_COLORS[e], alpha=0.05, lw=0)
        ax.text(pd.Timestamp(a) + (pd.Timestamp(b) - pd.Timestamp(a)) / 2,
                y_frac, e, transform=ax.get_xaxis_transform(),
                ha="center", va="top", fontsize=9, color=ERA_COLORS[e], weight="bold")
...
def main():
    ...
    a1.plot(tot.date, tot.total_oi_x4_pre.clip(0.5), color="#333333", lw=1.0)
    a1.set_yscale("log")
    a1.set_ylabel("total open interest\n(contracts, split-adjusted)")
    a1.set_title("One wall, 173 lives: the $50 call strike has never been empty since its birth; "
                 "every line below it dies on schedule", fontsize=12, weight="bold", pad=10)
    shade_eras(a1)
    ...
    for cls, lab in [("REINCARNATE", "death → rebuilt (reincarnate)"), ("ROLL", "death → roll-like"),
                     ("DIE_NO_REBUILD", "death, no rebuild"), ("PANEL_EDGE", "panel edge")]:
        a2.scatter([], [], s=30, color=CLS_COLORS[cls], label=lab)
    a2.legend(loc="upper left", fontsize=8, framealpha=0.9)
    a2.set_ylabel("expiry date")
    a2.set_xlabel("store date (daily OI snapshot; OI on date d reflects end of day d−1)")
    for ev, lab in [("2022-07-22", "4:1 split"), ("2025-10-07", "warrant reset")]:
        ...
        a2.annotate(lab, (pd.Timestamp(ev), 0.02), xycoords=("data", "axes fraction"),
                    fontsize=8, rotation=90, va="bottom", ha="right", color="k")
